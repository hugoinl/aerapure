...
// ---- Hero media detector: add 'has-media' class only when media is ready ----
document.addEventListener('DOMContentLoaded', function(){
  var hero = document.querySelector('.hero');
  if(!hero) return;
  var vid = hero.querySelector('.hero-bg');
  var img = hero.querySelector('.hero-bg-fallback');

  function activate(){ hero.classList.add('has-media'); }
  function fallback(){ /* keep text dark if nothing loads */ }

  try{
    if(vid){
      if(vid.readyState >= 2) activate();
